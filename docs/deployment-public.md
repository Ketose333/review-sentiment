...
목표는 새 스택(FastAPI + Next.js)이 기존 Streamlit 데모를 **대체**하는 것이다. [이관 완료 기준](migration-parity.md)은 세 모델 예측·설명과 500자 입력이다. 로컬 컨테이너와 공개 환경에서 세 모델의 예측·설명과 500자 경계를 확인했고, 2026-09-26 운영 측정과 동시성 계약 확정으로 이관 완료를 판정했다(이슈 #23). 월 단위 운영 관찰은 계속한다. 사용자가 2026-09-25 기존 리뷰 감성 분석 Streamlit 배포를 삭제했고, 이 저장소의 전용 keep-alive도 제거했다(이슈 #17). 배포 삭제와 이관 완료 판정은 별개다.
...
**판정:** 단일 사용자 기준으로 기능·지연·메모리는 무료 한도 안에서 동작한다. 월 방문 용량이 작고 동시 요청은 대기열로 처리된다. 대기열 동작은 2026-09-26 공개 API 계약으로 확정했다([api.md](api.md), 이슈 #23).

#### 메모리 요청 4GiB 비교 (이슈 #21) — 8GiB 유지

관측 최대 메모리가 요청의 약 22%라 4GiB로 줄여 초당 비용을 약 20% 낮추는 방안을 공개 환경에서 비교했다. 같은 측정 스크립트로 새 컨테이너의 첫 호출 추론 시간(`durationMs`, 모델 로드 포함)을 쟀다.

| 설정 | TF-IDF | LSTM 500자 | KLUE-BERT 500자 | 최대 메모리 |
| --- | --- | --- | --- | --- |
| 8GiB, 기존 이미지 | 2.2초 | 11.4초 | 9.1초 | 1.74GiB |
| 4GiB, 새 이미지 (2회) | 5.6·6.4초 | 18.3·19.6초 | 20.2·15.2초 | 2.06GiB |
| 8GiB, 같은 새 이미지 | 2.4초 | 11.9초 | 9.6초 | — |

메모리 값을 바꾸면 `backend/modal_app.py`가 이미지에 복사되어 이미지도 새로 빌드된다. 그래서 새 이미지 첫 사용의 불리함을 8GiB 쪽에 두고 같은 이미지로 다시 비교했다. 8GiB에서 원래 속도가 재현되었으므로 지연의 원인은 메모리 요청 축소다. 4GiB에서도 메모리 부족이나 실패는 없었다. 그러나 첫 분석이 모델별로 약 3~11초 늘어 컨테이너 가동 시간이 길어지고, 절감 효과도 줄어든다. 사용자 대기 시간을 우선해 **8GiB를 유지**하고 코드는 변경하지 않았다. 비교 과정에서 이미지 빌드 2회와 측정 3회를 수행했다.

동시성 결정(이슈 #23): 방문량이 적은 포트폴리오 운영이므로 Modal 입력 동시성과 컨테이너 수를 늘리지 않는다. 동시 요청은 거절 없이 순서대로 처리하는 것을 공개 계약으로 둔다. 입력 동시성을 늘리면 같은 컨테이너 안에서 LIME 작업이 겹쳐 메모리·지연을 다시 측정해야 하고, 컨테이너를 늘리면 무료 크레딧 소모가 커진다.
...
### 3단계 — 운영 게이트와 기존 배포 정리 (2026-09-26 완료)

세 모델의 기능 동등성과 새 주소의 안정성을 확인해 이관 완료 여부를 기록한다. 기존 리뷰 감성 분석 Streamlit 배포는 사용자가 2026-09-25 삭제했다. 같은 날 공개 주소가 존재하지 않는 앱과 동일한 인증 리다이렉트로 응답함을 확인하고 이 저장소의 전용 keep-alive workflow·스크립트·자동 검증을 제거했다(이슈 #17). 새 주소를 깨우는 자동 방문 작업은 계획하지 않는다. 2026-09-26 공개 운영 측정(콜드 스타트·지연·메모리·무료 사용량), 메모리 비교(8GiB 유지), 동시성 계약 확정을 거쳐 이관 완료로 기록했다(이슈 #19·#21·#23). 이는 하루 동안의 측정에 근거하며, 월 크레딧 소진·무료 정책 변경은 운영 중 계속 관찰한다.

Windows 로컬 측정에서 KLUE-BERT LIME은 30샘플 약 62초였으나, Linux 컨테이너 로컬 짧은 입력은 7.78초, 공개 API 짧은 입력은 11.009초였다([측정 기록](lime-measurement.md)). 입력 길이·CPU 할당·동시 부하에 따른 편차와 공개 프록시 기한, 강제 설명 실패 동작을 추가 검증해야 한다. 2026-09-26 공개 500자 설명은 세 모델 모두 성공했다(운영 설정 8GiB에서 5.7~17.6초). 강제 설명 실패는 재배포가 필요해 로컬 테스트로 대신했다.
